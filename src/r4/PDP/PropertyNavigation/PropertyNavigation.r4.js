...
import Breadcrumb_R3 from '../../../r3/components/BreadCrumb/Breadcrumb.r3';
...
export default class PropertyNavigation_R3 extends Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
        ...
        if (
            ...
            var _searchStringFormat =
                ...
                ) || '%(Line1)s, %(Line2)s, %(Locality)s, %(Region)s, %(PostCode)s',
            ...
            if (features.useCachedPlaces && features.useCachedPlaces.enabled && features.useCachedPlaces.enabled == true) {
                CachedPlaces.lookup({ address: _searchString, endpoint: features.useCachedPlaces.addressEndpoint + "?address=" + _addressSegments.PostCode + "&countryCode=" + _addressSegments.Country }, _cb, _cb);
            } else {
            ...
            if (features.useCachedPlaces && features.useCachedPlaces.enabled && features.useCachedPlaces.enabled == true) {
                CachedPlaces.lookup({ address: searchString, endpoint: features.useCachedPlaces.addressEndpoint + "?address=" + _addressSegments.PostCode + "&countryCode=" + _addressSegments.Country }, _cb, _cb);
            } else {
    ...
    createSearchContext(route, place) {
        ...
        if (
            ...
            if(this.context.stores.ParamStore.getParam("radius")){
                searchContext += '&initialRadius=' + this.context.stores.ParamStore.getParam("radius");
            }
        }
        ...
        switch (type) {
            case 'prev':
                if (renderPrev || outSideContext) {
                    ...
                    return (
                        <EventButton
                            className={`${this.props.btnClass} ${this.props.btnPrevClass
                                } ${noNextClass}`}
            ...
            case 'next':
                if (renderNext && !outSideContext) {
                    return (
                        <EventButton
                            className={`${this.props.btnClass} ${this.props.btnNextClass
                                }`}
                            listenFor="CURRENT_PROPERTY_UPDATED"
                            store={PropertyStore}
            ...
            <div>
                <React.Fragment>
                    ...
                    <Breadcrumb_R3
                        ...
                        spaPath = {this.context.spaPath.path}
                    />
                </React.Fragment>
                {navButtons}
            </div>
    ...
    render() {
        ...
        const renderPrev = PropertyStore.getCurrentPropertyIndex() > 1;
        const renderNext = PropertyStore.getCurrentPropertyIndex() < PropertyStore.getTotalResults();
        ...
        const _navButtons = this.context.stores.ParamStore.getSearchContext() ? (
            <div className={this.props.btnContainerClass}>
                ...
                {renderPrev && renderNext &&
                    <span className="propertyNavigationDivider"> | </span>
                }
...
PropertyNavigation_R3.contextTypes = {
...
PropertyNavigation_R3.propTypes = {
...
PropertyNavigation_R3.defaultProps = {
    location: {},
    address: {},
    breadcrumbPrefix: []
};
