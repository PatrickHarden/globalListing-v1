...
var React = require('react'),
    ...
    _ = require('lodash'),
    MapSearch = require('./MapSearch'),
    Select = require('./Select'),
    RangeFilter = require('./Range'),
    Input = require('./Input'),
...
import Filter from '../../list-map-components/Filter/Filter';
        ...
        return filters.map(
            function(filter) {
                ...
                if (view === 'map-list') {
                    ...
                    const param =
                        params[filter.name] ||
                        (filter.name) ? params[filter.name.toLowerCase()] : null;
                    ni++;
        ...
        if (
            ...
            if (view === 'map-list') {
                if (_searchMode === 'polygon') {
                    ...
                    multiSelectFilter.label = this.context.language.PolygonSearchPlaceholder;
                    multiSelectFilter.noResultsText = this.context.language.PolygonSearchNoResults;
                    var singlePolygonSearch = this.getConfigStore().getItem('features').singlePolygonSearch || false;

                    _jsx = (
                        <PolygonSearch
                            singlePolygonSearch={singlePolygonSearch}
                            handleFilterChange={this._regionsSelected}
                            filter={multiSelectFilter}
                            key="polygon-search-filter"
                            disabled={disabled}
